...
uint256 hashGenesisBlock("0x8f345dfb6614d115b9d3db83b5cd17c72ab65fed73b617b75d125c5a2ab60a83");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tubaclecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tubaclecoin Signed Message:\n";
...
{
    // Tubaclecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 6300000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 6300000); // Tubaclecoin: 6300k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Tubaclecoin: 3.5 days
static const int64 nTargetSpacing = 20; // Tubaclecoin: 20 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tubaclecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x72beaff5c67e73b30b4f981fd7ca60ca393cec472580ca5c78012f53f85e3423");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e56cd25ed0995b86f2a6db653e0db9aaf422c2211aa7d5249dc7d3639fd0d88b91205e4ea24a65b9b08d50c947c89967d19b65a4e059c68cd39d4204580a1be8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1599258894;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085469077;

        if (fTestNet)
        {
            block.nTime    = 1599258689;
            block.nNonce   = 385420275;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x937f3b5fa805c5971b81db6dee17b7bc6a4914a146d0e48d9a598763ff104667"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Tubaclecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TubaclecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TubaclecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TubaclecoinMiner(CWallet *pwallet)
{
    printf("TubaclecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tubaclecoin-miner");
    ...
    try { loop {
        ...
        printf("Running TubaclecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TubaclecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TubaclecoinMiner, pwallet));
}
